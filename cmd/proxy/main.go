...
`
	...
	go func() {
		// 这里是接收操作系统的信号量
		// 如果proxy进程被kill掉 会在这个线程中回调
		// 将会调用proxy.close方法，这个里面调用了Jodis的close方法，将zk上面的该proxy信息删除
	...
	//启动之后，Proxy会默认处于 waiting 状态，以一秒一次的频率刷新状态，监听proxy_addr 地址（默认配置文件中的19000端口）
	//但是不会 accept 连接，通过fe或者命令行添加到集群并完成集群状态的同步，才能改变状态为online
}
