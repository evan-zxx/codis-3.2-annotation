...
// 整个集群的上下文信息 包括当前group coids-server proxy slots信息
// 会在每个需要的时刻从store(zk)中刷新到内存中
// 如果上下文中的信息有变更, 会先更新内存中的context 然后再更新store(zk)
type context struct {
	// 槽映射关系
...
func (ctx *context) toSlot(m *models.SlotMapping, p *models.Proxy) *models.Slot {
	...
	case models.ActionNothing, models.ActionPending:
		...
		// 该slot属于的group以及replication group
	...
	case models.ActionPrepared:
		fallthrough
	case models.ActionMigrating:
		// 正在迁移的slot, 将其BackendAddr设置为迁移目的地的group
...
//为了主从读写分离设置的。从优先级我们可以看到，读请求会优先转发到和proxy在一台服务器上的codis-server，也就是优先级最高的0
...
// 将SlotMapping切片转化为Slot切片
// 在Slot结构中记录了这个Slot在迁移的不同阶段，并且设置相应的BackendAddr
